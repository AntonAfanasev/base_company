...
Создаем БД PostgreSQL и немного меняем настройки в Django-проекте:
...
Скачиваем virtualenv и другие пакеты для проекта:
...
Создаем ссылку от уже подготовленного файла company_nginx.conf (на одном уровне с manage.py) в /etc/nginx/sites-enabled:
...
Тестируем, видит ли nginx медиа и статику.
...
Деактивируем виртуальное окружение командой:
deactivate
...
Осталось добавить в файл /etc/rc.local перед строкой “exit 0” следующее:
/usr/local/bin/uwsgi --emperor /etc/uwsgi/vassals --uid UID_ПОЛЬЗОВАТЕЛЯ --gid GID_ПОЛЬЗОВАТЕЛЯ

Вводим sudo reboot и смотрим, как наш сайт автоматически запускается после перезагрузки сервера.
